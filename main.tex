...
"It is not possible to prepare a report without the assistance of other people. This one is certainly no exception."\newline

The completion of this report could not have been accomplished without the support of an outstanding group of people, and we would like to thank every one of them.\newline

We would like to express our sincere and deep gratitude towards Lakshmi Burra Ma'am, who instilled in us the ability to develop analytical and abstract thinking. She, through her enthusiasm, has always encouraged and supported us. It's her constant and conscientious guidance, which has helped us complete this report.\newline

Our heartfelt thanks to our TAs, who have provided their invaluable assistance and advised us on every step of this report. \newline
...
\newpage
\begin{center}
    {\Large Probability and Measures}
\end{center}
...
For example:- \newline
1. Throwing a dice : $\Omega$ = $\{1,2,3,4,5,6\}$\newline
2. Tossing a coin twice : $\Omega$ = $\{HH, TT, HT, TH\}$, where 'H' represents that the heads and 'T' represents the tails.\newline
...
NOTE - The sample space of an experiment can be countable or uncountable. The sample space of an experiment like tossing a coin n times is countable with 2$^{n}$ outcomes whereas if we want to determine the speed with which the coin tossed would land back is a set of positive real numbers and the sample space would be uncountable. Another example of uncountable sample space could be the set of all points on the circumference of a circle.\newline
\newpage
...
1. $\mu(\phi)$ = 0\newline
2. If $A_{1}, A_{2} ,A_{3} ....$ are a countable collection of disjoint measurable sets, then \newline $\mu(\bigcup_{1}^{\infty} {A_k}) = \sum_{1}^{\infty}\mu(A_k)$. This is referred to as the countable additivity axiom.\newline
\newpage
Measure is nothing but a mapping of the subsets included in $\Sigma$ to [0, $\infty$].\newline
We say that
\newline
1. if $\mu(\Omega)$ $<$ $\infty$ , $\mu$ is called a finite measure.\newline
2. if $\mu(\Omega)$ = $\infty$ , $\mu$ is called a infinite measure.\newline
3. if $\mu(\Omega)$ =  1 , $\mu$ is called a probability measure.\newline \newline
So a probability measure is a special case of finite measure with the measure of the $\Omega$ being 1.\newline

\subsection* {Probability Measure}
We can define a probability measure($P$) on ($\Omega$, $\Sigma$) as a function $P$: $\Sigma$ $\rightarrow$ [0,1] such that - \newline
1. $P(\phi)$ = 0\newline
2. $P(\Sigma)$ = 1\newline
3. if $A_{1}, A_{2} .... $ are $\Sigma$ measurable sets then $P(\bigcup_{1}^{\infty} {A_k}) = \sum_{1}^{\infty}P(A_k)$. \newline
...
1. $P(A) \le 1$\newline
2. $P(A^{c})$ = 1 - $P(A)$ $\forall A \epsilon \Sigma$\newline
3. if $A=B$, then $P(A) = P(B)$\newline
4. Monotonocity: if $A \subset B \Rightarrow P(A) \le P(B)$\newline
5.$P(B\cap A^{c}) = P(B) - P(A\cap B)$\newline
6.$P(A\cup B) = P(A) + P(B) - P(A\cap B)$\newline
7. Finite additivity:\hfill If $A_{1}, A_{2} ... A_{n} \epsilon \Sigma$ and suppose they are disjoint. Then - $P(\bigcup_{1}^{n} {A_k}) = \sum_{1}^{n}P(A_k)$.\newline
\newpage
\section{References}
\begin{itemize}
