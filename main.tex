...
We can also see that two sets having the same number of points need not have the same measure, let $A=[0,1]$ and $B=[0,2]$, there exists a bijection from A to B and have the same cardinality, but B is twice as long as A.
 
 Remarkably, it turns out such a measure $does \, not \,exist$. This is quite a surprising fact, mostly because it is counter-intuitive. These examples tell us that it is impossible to measure every subset of $R^n$ applying all the above properties. 
...
Proof: Suppose $m^*(A) = M < \infty$. Then for all $\epsilon > 0 $ there exist ${I_n}$ bounded open intervals, such that they cover A. And from sec 1.1.7. $\sum l(I_n) < M+ \epsilon$, so if $y \in R$, then ${I_{n}+y}$ is a covering of A+y and so $m^*(A+y) \leq M$
...
The proof is similar to the ones seen above, solve while taking each condition separately.
