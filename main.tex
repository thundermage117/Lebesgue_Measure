...
\subsection{The outer measure of any interval is it's length.}
...
\subsection{$[0,1]$ is not countable}
...
\begin{enumerate}
	...
	\item If A$_i$ $\in$ $\Sigma$, $i$ $\in$ $\mathbb{N}$, then $\bigcup\limits_{i=1}^n$A$_i$ $\in$ $\Sigma$ \textit{(\textit{property 3})}
\end{enumerate}
